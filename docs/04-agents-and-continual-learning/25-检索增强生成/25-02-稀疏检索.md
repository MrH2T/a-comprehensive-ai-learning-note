---
title: "25.2 稀疏检索"
source_docx: "第4部分 大模型智能体与持续学习/25.检索增强生成/25.2 稀疏检索.docx"
status: "auto-converted"
ocr: "no pending image placeholders in public Markdown"
license: "CC BY-NC-SA 4.0"
local_only: false
---

# 25.2 稀疏检索


稀疏检索即传统关键词检索方法，如BM25：

BM25 的核心思想是通过三个维度衡量查询 $Q$ 与文档 $D$ 的相关性：

1. 词频（TF, Term Frequency）：词在文档中出现的次数越多，相关性越高，但有饱和上限。

2. 逆文档频率（IDF, Inverse Document Frequency）：包含该词的文档越少，该词的权重（信息量）越大。

3. 文档长度归一化（Document Length Normalization）：同样的词频，出现在短文档中比出现在长文档中更重要。

假设查询 $Q$ 由单词 $\{q_1, q_2, \ldots, q_n\}$ 组成，文档 $D$ 的 BM25 得分计算公式如下：

$$
\mathrm{Score}(D,Q)=\sum_{i=1}^{n}\mathrm{IDF}(q_i)\cdot \frac{f(q_i,D)\cdot(k_1+1)}{f(q_i,D)+k_1\cdot(1-b+b\cdot\frac{|D|}{\mathrm{avgdl}})}
$$

其中：

- $f(q_i,D)$：单词 $q_i$ 在文档 $D$ 中的出现频率。
- $|D|$：文档 $D$ 的长度（单词数）。
- $\mathrm{avgdl}$：语料库中文档的平均长度。
- $k_1$：词频饱和参数，通常取 $1.2\sim2.0$，控制 TF 增长带来的得分上限。
- $b$：长度惩罚参数，通常取 $0.75$，决定文档长度对得分的影响程度。

$\mathrm{IDF}(q_i)$ 的标准计算方式为：

$$
\mathrm{IDF}(q_i)=\ln\left(\frac{N-n(q_i)+0.5}{n(q_i)+0.5}+1\right)
$$

其中 $N$ 是语料库文档总数，$n(q_i)$ 是包含单词 $q_i$ 的文档数。

## 参考文献

- Robertson, S., & Zaragoza, H. (2009). [The Probabilistic Relevance Framework: BM25 and Beyond](https://doi.org/10.1561/1500000019). Foundations and Trends in Information Retrieval, 3(4), 333-389.
- Manning, C. D., Raghavan, P., & Schutze, H. (2008). [Introduction to Information Retrieval: Okapi BM25, a non-binary model](https://nlp.stanford.edu/IR-book/html/htmledition/okapi-bm25-a-non-binary-model-1.html). Cambridge University Press.
