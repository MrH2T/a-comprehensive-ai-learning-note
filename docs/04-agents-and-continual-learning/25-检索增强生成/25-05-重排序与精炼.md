...
可以看到，这里并非Query对文档用互注意力（Bi-Encoder），而是Query和文档拼接后用自注意力打分（Cross-Encoder）。最后取 `[CLS]` 标记对应的全局注意力向量，追求极致的精度。（注：$W$ 为 $1\times d_{\mathrm{model}}$ 维，和 `[CLS]` token 向量的 $d_{\mathrm{model}}\times 1$ 维相乘为标量）
