...
1. 每个训练 batch 同时从 $\mathcal D_{\mathrm{env}}$ 和 $\mathcal D_{\mathrm{model}}$ 采样。
2. 模型数据占比可以较高，例如 95%，真实数据用于校准模型生成数据的偏差。
3. 每个真实环境步之后，可以执行多次 SAC 更新，例如 20 次。
4. 这样既利用模型数据提升样本效率，又保留 SAC 在连续控制任务中的稳定优化能力。
